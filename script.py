...
def download_video_sync(url: str, output_path: str, quality_format: str) -> str:
    """Функція завантаження відео з будь-якого джерела через yt-dlp."""
    ydl_opts = {
...
async def start_command(client: Client, message: Message):
    await message.reply_text(
        ...
        "Завдяки Telegram Client API я можу надсилати великі файли **розміром до 2 ГБ**!\n\n"
...
async def process_quality_selection(client: Client, callback: CallbackQuery):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            # Відправка файлу (Pyrogram підтримує до 2 ГБ!)
            if ext == "mp3":
        ...
        await status_msg.edit_text("❌ Сталася помилка при завантаженні відео. Переконайся, що посилання публічне.")
